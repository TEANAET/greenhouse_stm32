...
void WiFi_EnableIO_Init()
{
  GPIO_InitTypeDef GPIO_InitStructure;                    //定义一个设置IO端口参数的结构体
	...
	GPIO_Init(GPIOG, &GPIO_InitStructure);            	    //设置PE6
	
	GPIO_SetBits(GPIOG,GPIO_Pin_7);													//使能PD/EN,高电平
}	
/*-------------------------------------------------*/
/*函数名：初始化ESP8266的复位IO                       */
/*参  数：无                                       */
...
void WiFi_ResetIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;               //准备设置PE6
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;       //速率50Mhz
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;   	    //推挽输出方式
	GPIO_Init(GPIOE, &GPIO_InitStructure);            	    //设置PE6
	RESET_IO(1);                                            //复位IO拉高电平
}
/*-------------------------------------------------*/
/*函数名：WiFi发送设置指令                          */
